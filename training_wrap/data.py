...
class DataProvider:
    ...
    def get_batch(self, ind):
        ...
        if (start > end):
            _data = np.concatenate((self.data[:end], self.data[start:self.train_len]), axis=0)[perm]
            for k in self.label_keys:
                _labels[k] = np.concatenate((self.labels[k][:end], self.labels[k][start:self.train_len]), axis=0)[perm]
        else:
    ...
    def get_batch_test(self, ind):
        ...
        if (start > end):
            _data = np.concatenate((self.data[self.train_len:end], self.data[start:]), axis=0)[perm]
            for k in self.label_keys:
                _labels[k] = np.concatenate((self.labels[k][self.train_len:end], self.labels[k][start:]), axis=0)[perm]
        else:
            _data = self.data[start:end][perm]
            for k in self.label_keys:
                _labels[k] = self.labels[k][start:end][perm]
